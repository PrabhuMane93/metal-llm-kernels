...
// actually linked into the binary — same fix as gemv/tiled_gemv.
...
fn main() {
    ...
    let source = NSString::from_str(include_str!("../gemv_q4_fused.metal"));
    let library = device
        .newLibraryWithSource_options_error(&source, None)
        .expect("shader compile failed");
    let function = library
        .newFunctionWithName(&NSString::from_str("gemv_q4_fused"))
        .expect("gemv_q4_fused kernel not found in library");
    let pipeline = device
    ...
    let x_buffer = device
        .newBufferWithLength_options(
            (num_cols * size_of::<f32>()) as NSUInteger,
